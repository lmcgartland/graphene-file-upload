"""Base module for file based graphql test shared by both django and flask."""
...
def file_graphql_query(
    ...
    if not client:
        raise ValueError('Missing required argument: "client".')
    ...
    data.update(files)
    if headers:
        resp = client.post(graphql_url, data=data, **headers)
    else:
        resp = client.post(graphql_url, data=data)
